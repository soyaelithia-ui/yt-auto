...
def is_profiling_enabled() -> bool:
    ...
    if env_val != _PROFILING_ENV_SNAPSHOT:
        ...
        _PROFILING_ENABLED_CACHED = env_val.strip().lower() not in ("0", "false", "no", "off", "disable", "disabled")
...
class ProfilingSummary:
    ...
    def format_table(self, color: bool = True) -> str:
        ...
        lines = ["=" * 110, f" PIPELINE PROFILING REPORT — Run: {self.run_id or 'N/A'} | Channel: {self.channel or 'N/A'} | Story: {self.story_id or 'N/A'}", "=" * 110]
        lines.append(f" {'#':<3} {'Stage Name':<28} {'Duration (s)':>12} {'% Time':>8} {'CPU User':>10} {'CPU Sys':>10} {'CPU %':>8} {'RSS Δ (MB)':>11} {'Peak RSS':>10} {'Status':>7}")
        lines.append("-" * 110)
    ...
    def get_phase(self, stage_name: str | CanonicalStage) -> PhaseMetrics | None:
        ...
        with self._lock:
            for p in self._phases:
                if p.stage_name == target or normalize_stage_name(p.stage_name) == target or str(stage_name).lower() in p.stage_name.lower():
                    return p
        return None
