    ...
    *,
    ...
    if visual_plan_path is None:
        ...
        try:
            ...
            is_loop_plan = (
                (video_engine is not None and str(video_engine).lower() in ("loop", "loop_video", "loop_video_engine", "loop_compositor", "video_loop"))
                or str(plan.get("video_engine", "")).lower() in ("loop", "loop_video", "loop_video_engine", "loop_compositor", "video_loop")
                or plan.get("loop") is True
                or plan.get("is_loop") is True
                or (len(scenes) == 1 and str(plan.get("mode", "")).lower() == "loop")
                or (isinstance(precomputed_visual, dict) and precomputed_visual.get("engine") in ("loop", "video_loop"))
            )
