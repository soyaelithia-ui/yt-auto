...
| **`AgentModelResolutionError` / `NotFoundError`** | Ninguna preferencia Luna ni fallback aparece en `agy models`. | `agy models`; revisar `AGY_MODEL_PREFERENCES` y `AGY_FREE_FALLBACK_MODEL`. | Elegir un identificador listado; el arnés no envía modelos no disponibles. |
