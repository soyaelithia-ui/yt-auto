...
| **`AgentModelResolutionError` / `NotFoundError`** | Ninguna preferencia de modelo ni fallback aparece en `agy models`. | `agy models`; revisar `AGY_MODEL_PREFERENCES` y `AGY_FREE_FALLBACK_MODEL`. | Elegir un identificador listado; el arnés no envía modelos no disponibles. |
...
| **Saturación / HTTP 429 en Agentes** | Cuota o límite de velocidad alcanzado en la cuenta Antigravity. | Logs del `CircuitBreaker` o estado `saturated` | Enfriamiento de 300s y fallback narrativo/SEO sin detener el demonio. |
