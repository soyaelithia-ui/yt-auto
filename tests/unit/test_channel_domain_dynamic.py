...
def test_canonical_channel_thematic_channels():
    ...
    assert canonical_channel(CanonicalChannel.HORROR) == CanonicalChannel.HORROR
    assert canonical_channel(CanonicalChannel.DRAMA) == CanonicalChannel.DRAMA
    assert canonical_channel(CanonicalChannel.SCIFI) == CanonicalChannel.SCIFI
...
def test_channel_settings_dynamic_env_override(monkeypatch):
    ...
    monkeypatch.setenv("HORROR_HANDLE", "@TestHorrorHandleEnv")
    monkeypatch.setenv("HORROR_NAME", "TestHorrorNameEnv")
    ChannelProfileRegistry._ensure_loaded(force_reload=True)

    ch = _resolve_dynamic_channel("horror")
    assert ch.handle == "@TestHorrorHandleEnv"
    assert ch.public_name == "TestHorrorNameEnv"

    # Verify SETTINGS.channel and get_channel_settings also resolve dynamically
    ch_settings = SETTINGS.channel("horror")
    assert ch_settings.handle == "@TestHorrorHandleEnv"
    assert ch_settings.public_name == "TestHorrorNameEnv"

    ch_facade = get_channel_settings("horror")
    assert ch_facade.handle == "@TestHorrorHandleEnv"
    assert ch_facade.public_name == "TestHorrorNameEnv"

    # Verify SETTINGS.channels mapping resolves dynamically
    ch_dict = SETTINGS.channels[CanonicalChannel.HORROR]
    assert ch_dict.handle == "@TestHorrorHandleEnv"
    assert ch_dict.public_name == "TestHorrorNameEnv"
...
def test_channel_settings_generic_channel_handle_override(monkeypatch):
    from src.config import SETTINGS, get_channel_settings, _resolve_dynamic_channel
    monkeypatch.setenv("CHANNEL_KEY", "drama")
    monkeypatch.setenv("CHANNEL_HANDLE", "@GenericDrama")
    monkeypatch.setenv("CHANNEL_NAME", "Generic Drama")
    ChannelProfileRegistry._ensure_loaded(force_reload=True)

    ch = _resolve_dynamic_channel("drama")
    assert ch.handle == "@GenericDrama"
    assert ch.public_name == "Generic Drama"

    ch_settings = SETTINGS.channel("drama")
    assert ch_settings.handle == "@GenericDrama"
    assert ch_settings.public_name == "Generic Drama"

    ch_facade = get_channel_settings("drama")
    assert ch_facade.handle == "@GenericDrama"
    assert ch_facade.public_name == "Generic Drama"
...
def test_settings_channels_dynamic_iteration():
    ...
    assert CanonicalChannel.HORROR in active_keys
...
def test_channel_profile_registry_audio_and_auth_env_overrides(monkeypatch):
    monkeypatch.setenv("HORROR_TTS_VOICE", "es-ES-CustomVoice")
    monkeypatch.setenv("HORROR_YOUTUBE_CHANNEL_ID", "UC_CUSTOM_HORROR_123")
    monkeypatch.setenv("HORROR_SOURCE_FEED", "r/custom_feed")
    ChannelProfileRegistry._ensure_loaded(force_reload=True)

    prof = ChannelProfileRegistry.get_channel("horror")
    assert prof.audio.default_voice_profile == "es-ES-CustomVoice"
    assert prof.auth.expected_youtube_channel_id == "UC_CUSTOM_HORROR_123"
...
def test_canonical_channel_bidirectional_resolution():
    """Bidirectional resolution maps canonical identifiers cleanly."""
    assert canonical_channel("horror") == CanonicalChannel.HORROR
    assert canonical_channel("drama") == CanonicalChannel.DRAMA
    assert canonical_channel("scifi") == CanonicalChannel.SCIFI
