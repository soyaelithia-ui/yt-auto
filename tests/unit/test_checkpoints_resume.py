...
def _seed_story(repo, story_id="story-1", channel="moku"):
...
def test_record_checkpoint_stamps_story_key_for_cross_run_lookup(repo):
    ...
    cp.record_checkpoint(repo, run_a, "story-1:moku-scp-shorts", "audio", artifact)
    ...
    assert row["story_key"] == "story-1:moku-scp-shorts"
...
def test_resume_plan_reuses_valid_prefix(tmp_path):
    ...
    plan = cp.resume_plan(str(repository.db_path), "story-1", "moku-scp-shorts")
...
def test_resume_plan_stops_at_first_missing_stage(tmp_path):
    ...
    plan = cp.resume_plan(str(repository.db_path), "story-2", "moku-horror-long")
...
def test_resume_plan_detects_corrupted_artifact_by_sha(tmp_path):
    ...
    plan = cp.resume_plan(str(repository.db_path), "story-3", "moku-scp-shorts")
...
def test_pipeline_copies_reusable_artifacts_into_new_workdir(tmp_path, monkeypatch):
    ...
    repository = QueueRepository(tmp_path / "queue.db")
    repository.initialize()
    repository.enqueue("story-x", "SCP-173: La Escultura", "Historia completa de prueba.", "https://x/1", "moku")
...
def test_resumable_video_requires_faststart_container(tmp_path):
    ...
    with sqlite3.connect(repository.db_path) as conn:
        conn.execute(
            ...
            " VALUES ('r1','moku','s9','publish','RETRYABLE_FAILED','w',datetime('now'),datetime('now'))"
        )
        conn.execute(
