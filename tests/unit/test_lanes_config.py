...
class TestResolution:
    ...
    def test_cross_channel_lane_rejected(self):
        with pytest.raises(ValueError, match="no existe o no pertenece"):
            resolve_lane_for_run("aelithia", "horror-scp-shorts", path="no/such/file")
    ...
    def test_lanes_for_channel_filters(self):
        ...
        assert all(lane.channel.value == "horror" for lane in moku)
    ...
    def test_thematic_lane_aliases_resolve_get_lane(self):
        ...
        assert get_lane("moku-scp-shorts") is not None
        assert get_lane("moku-scp-shorts").id == "horror-scp-shorts"
        assert get_lane("horror-long") is not None
        assert get_lane("horror-long").id == "horror-horror-long"
        assert get_lane("moku-horror-long") is not None
        assert get_lane("moku-horror-long").id == "horror-horror-long"
        assert get_lane("drama-shorts") is not None
        assert get_lane("drama-shorts").id == "drama-drama-shorts"
        assert get_lane("aelithia-drama-shorts") is not None
        assert get_lane("aelithia-drama-shorts").id == "drama-drama-shorts"
        assert get_lane("drama-aita-long") is not None
        assert get_lane("drama-aita-long").id == "drama-aita-long"
        assert get_lane("aelithia-aita-long") is not None
        assert get_lane("aelithia-aita-long").id == "drama-aita-long"
